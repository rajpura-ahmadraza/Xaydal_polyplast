...
class OrdersView extends GetView<BizController> {
      ...
      body: Column(children: [
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: sel ? C.g700 : (dark ? C.s700 : C.s300)),
                ),
                child: Text(tabs[i], style: GoogleFonts.poppins(
        ...
        Expanded(child: Obx(() {
          ...
          if (list.isEmpty) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.receipt_long_outlined, size: 60, color: dark ? C.s600 : C.s400),
            const SizedBox(height: 12),
            Text('No orders found', style: GoogleFonts.poppins(fontSize: 15, color: C.s500)),
          ]));
          ...
          return ListView.separated(
            ...
            itemBuilder: (_, i) => _card(list[i], fmt, dark),
          );
        })),
      ]),
  ...
  Widget _card(OrderModel o, NumberFormat fmt, bool dark) {
    final isPending   = o.status == 'pending';
    final isDelivered = o.status == 'delivered';
    final color = isDelivered ? C.green : isPending ? C.orange : C.red;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
            ...
            child: Icon(
              isDelivered ? Icons.check_circle_rounded : isPending ? Icons.hourglass_top_rounded : Icons.cancel_rounded,
              color: color, size: 22,
            ),
          ),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(o.customerName, style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 15)),
            Text(o.customerPhone, style: GoogleFonts.poppins(fontSize: 12, color: C.s500)),
          ])),
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text('\u20B9${fmt.format(o.totalAmount.toInt())}',
                style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w700, color: C.g700)),
            Container(
        ...
        const SizedBox(height: 12),
        Row(children: [
          Icon(Icons.inventory_2_outlined, size: 14, color: C.s500),
          const SizedBox(width: 4),
          Text('${o.quantity}× ${o.productName}', style: GoogleFonts.poppins(fontSize: 12, color: C.s600)),
          const Spacer(),
          Icon(Icons.schedule_outlined, size: 14, color: C.s500),
          const SizedBox(width: 4),
          Text(dateStr, style: GoogleFonts.poppins(fontSize: 11, color: C.s500)),
        ]),
        if (o.notes != null && o.notes!.isNotEmpty) ...[
          const SizedBox(height: 8),
          Row(children: [
            Icon(Icons.notes_rounded, size: 14, color: C.s500),
            const SizedBox(width: 4),
            Expanded(child: Text(o.notes!, style: GoogleFonts.poppins(fontSize: 12, color: C.s500))),
          ]),
        ],
        if (isPending) ...[
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _btn('Mark as Delivered', C.green, () => controller.markDelivered(o.id))),
            const SizedBox(width: 10),
            Expanded(child: _btn('Cancel Order', C.red, () => controller.cancelOrder(o.id), outline: true)),
          ]),
        ],
      ]),
    );
