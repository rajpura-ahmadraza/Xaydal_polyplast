...
class DashboardView extends GetView<BizController> {
    ...
    return Scaffold(
      body: Obx(() => CustomScrollView(slivers: [
        ...
        SliverAppBar(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.end, children: [
                    Row(children: [
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Obx(() => Text('Hello, ${auth.userName.value.isEmpty ? 'Owner' : auth.userName.value} 👋',
                            style: GoogleFonts.poppins(fontSize: 14, color: Colors.white70))),
                        Obx(() => Text(auth.userBiz.value.isEmpty ? 'Plastic Business' : auth.userBiz.value,
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          gradient: const LinearGradient(colors: [C.g300, C.g500]),
                    ...
                    Container(
                      ...
                      child: Row(children: [
                        ...
                        Text('This Month: \u20B9${fmt.format(controller.monthEarning.toInt())}',
                        ...
                        Container(
                          ...
                          child: Text('+12%', style: GoogleFonts.poppins(fontSize: 11, color: Colors.greenAccent, fontWeight: FontWeight.w600)),
                        ),
                      ]),
                    ),
                  ]),
          ...
          title: Text('Dashboard', style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 17)),
          actions: [
        ...
        SliverToBoxAdapter(child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            // ── Metric Cards ──────────────────────────────────
            Row(children: [
              _metric("Today's Earning", '\u20B9${fmt.format(controller.todayEarning.toInt())}',
                  Icons.today_rounded, C.g700, dark),
              ...
              Container(
                ...
                child: Row(children: [
                  Container(padding: const EdgeInsets.all(8),
                      decoration: BoxDecoration(color: C.orange.withOpacity(0.15), borderRadius: BorderRadius.circular(10)),
                      child: const Icon(Icons.warning_amber_rounded, color: C.orange, size: 20)),
                  const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('Low Stock Alert!', style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 13, color: C.orange)),
                    Text(controller.lowStock.map((p) => '${p.name} (${p.stock} left)').join(', '),
                        style: GoogleFonts.poppins(fontSize: 12, color: C.orange), overflow: TextOverflow.ellipsis, maxLines: 2),
                  ])),
                ]),
              ),
            ],
            ...
            const SizedBox(height: 12),
            Row(children: [
              _action('New Order',    Icons.add_shopping_cart_rounded, C.g700,   () => Get.toNamed(AppRoutes.addOrder),    dark),
              const SizedBox(width: 10),
              _action('Add Product',  Icons.add_box_rounded,           C.teal,   () => Get.toNamed(AppRoutes.addProduct),  dark),
              const SizedBox(width: 10),
              _action('Add Customer', Icons.person_add_rounded,        C.indigo, () => Get.toNamed(AppRoutes.addCustomer), dark),
            ]),
  ...
  Widget _metric(String label, String value, IconData icon, Color color, bool dark) =>
      Expanded(child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 12),
          Text(value, style: GoogleFonts.poppins(fontSize: 21, fontWeight: FontWeight.w700)),
          Text(label, style: GoogleFonts.poppins(fontSize: 11, color: C.s500)),
        ]),
      ));

  Widget _action(String label, IconData icon, Color color, VoidCallback onTap, bool dark) =>
  ...
  Widget _sectionHeader(String title, String action) => Row(
    ...
    children: [
      ...
      Text(action, style: GoogleFonts.poppins(fontSize: 13, color: C.g700, fontWeight: FontWeight.w500)),
    ],
  );
  ...
  Widget _productRow(ProductModel p, bool dark) => Container(
    ...
    child: Row(children: [
      ...
      const SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(p.name, style: GoogleFonts.poppins(fontWeight: FontWeight.w500, fontSize: 14)),
        const SizedBox(height: 4),
        ClipRRect(borderRadius: BorderRadius.circular(4),
          child: LinearProgressIndicator(
            value: (p.stock / (p.minStock * 5)).clamp(0.0, 1.0), minHeight: 5,
            backgroundColor: dark ? C.s700 : C.s200,
            valueColor: AlwaysStoppedAnimation(p.isLowStock ? C.orange : C.g700),
          )),
        const SizedBox(height: 2),
        Text('${p.stock} pcs  ·  \u20B9${p.price.toInt()}/pc', style: GoogleFonts.poppins(fontSize: 11, color: C.s500)),
      ])),
      const SizedBox(width: 10),
  ...
  Widget _orderRow(OrderModel o, NumberFormat fmt, bool dark) {
    ...
    final color = delivered ? C.green : o.status == 'pending' ? C.orange : C.red;
    return Container(
      margin: const EdgeInsets.only(bottom: 10),
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(
      ...
      child: Row(children: [
        Container(
          ...
          child: Icon(delivered ? Icons.check_circle_rounded : Icons.pending_rounded, color: color, size: 24),
        ),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(o.customerName, style: GoogleFonts.poppins(fontWeight: FontWeight.w600, fontSize: 14)),
          Text('${o.quantity}× ${o.productName}', style: GoogleFonts.poppins(fontSize: 12, color: C.s500)),
        ])),
