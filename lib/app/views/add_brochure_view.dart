...
class AddBrochureView extends GetView<BizController> {
  ...
  Widget build(BuildContext context) {
    ...
    final priceCtrl  = TextEditingController(text: existing?.sellingPrice.toInt().toString() ?? '');
    final imgCtrl    = TextEditingController(text: existing?.imageUrl ?? '');
    final descCtrl   = TextEditingController(text: existing?.description ?? '');
    final matCtrl    = TextEditingController(text: existing?.material ?? 'HDPE Plastic');
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            Obx(() => GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: dark ? C.s700 : C.g200, width: 1.5),
                ),
                child: previewUrl.value.isNotEmpty
                    ? ClipRRect(borderRadius: BorderRadius.circular(16),
                        child: Image.network(previewUrl.value, fit: BoxFit.contain,
            ...
            _section(dark, 'Bucket Information', Icons.water_drop_outlined, C.g700, [
              _field(nameCtrl, 'Bucket Name (e.g. 10L Premium Bucket)', Icons.label_outline, dark,
                  validator: (v) => (v?.trim().isEmpty ?? true) ? 'Please enter bucket name' : null),
              const SizedBox(height: 12),
              Row(children: [
                Expanded(child: _field(sizeCtrl, 'Capacity (Litres)', Icons.water_outlined, dark,
                    type: TextInputType.number,
                    validator: (v) => (v?.trim().isEmpty ?? true) ? 'Required' : null)),
                const SizedBox(width: 12),
                Expanded(child: _field(matCtrl, 'Material', Icons.layers_outlined, dark)),
              ]),
            ]),
            const SizedBox(height: 14),

            _section(dark, 'Weight Specifications', Icons.scale_outlined, C.teal, [
              Row(children: [
                Expanded(child: _field(bodyCtrl, 'Body Weight (e.g. 220g)', Icons.scale_outlined, dark,
                    validator: (v) => (v?.trim().isEmpty ?? true) ? 'Required' : null)),
                const SizedBox(width: 12),
                Expanded(child: _field(lidCtrl, 'Lid Weight (e.g. 45g)', Icons.layers_outlined, dark,
                    validator: (v) => (v?.trim().isEmpty ?? true) ? 'Required' : null)),
              ]),
            ]),
            const SizedBox(height: 14),

            _section(dark, 'Pricing & Media', Icons.currency_rupee_rounded, C.indigo, [
              _field(priceCtrl, 'Selling Price (₹)', Icons.currency_rupee_rounded, dark,
                  type: TextInputType.number,
                  validator: (v) {
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  ...
                  prefixIcon: const Padding(padding: EdgeInsets.only(bottom: 40), child: Icon(Icons.notes_rounded, size: 20)),
                  prefixIconColor: dark ? C.g400 : C.g700,
                  filled: true, fillColor: dark ? C.s750 : C.s100,
                  border: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide.none),
                  enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide.none),
                ...
                onPressed: () {
                  if (formKey.currentState!.validate()) {
                    final item = BrochureItem(
                      ...
                      material: matCtrl.text.trim().isEmpty ? 'HDPE Plastic' : matCtrl.text.trim(),
                    );
                    if (isEditing) {
  ...
  Widget _noImage(bool dark) => Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    Icon(Icons.add_photo_alternate_outlined, size: 36, color: dark ? C.s600 : C.g300),
    const SizedBox(height: 8),
    Text('Enter image URL below to preview', style: GoogleFonts.poppins(fontSize: 12, color: dark ? C.s600 : C.g600)),
  ]);
