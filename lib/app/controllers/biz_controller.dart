...
class BizController extends GetxController {
  ...
  List<int> get monthlyOrderCount {
    ...
    return List.generate(6, (i) {
      ...
      return orders
          .where((o) => o.date.year == m.year && o.date.month == m.month)
          .length;
    });
  }
  ...
  CollectionReference get _brochureRef =>
      _db.collection('users').doc(uid).collection('brochure');
  ...
  Future<void> cancelOrder(String orderId) async {
    try {
      ...
      batch.update(_productsRef.doc(order.productId),
  ...
  // ── Brochure CRUD ────────────────────────────────────────────
  Stream<List<BrochureItem>> brochureStream() => _brochureRef
      .snapshots()
      .map((s) => s.docs.map((d) => BrochureItem.fromDoc(d)).toList());
